...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    mobilephone:{
    	validator:function(value,param){
    		var reg= /^((1[3|5|7|8]{1}\d{9})|((\d{3,4}\-)?\d{5,9}(\-\d{3,5})?))$/;
    		if(!reg.test(value)){
    			return false;
    		}
    		return true;
    ...
    len:{
    	validator:function(value,param){
    		return (!!value?value.length:0) >= param[0] && (!!value?value.length:0) <= param[1];
    	},
    ...
    ajbh:{
    	...
    	message:'为A开头+12位部门编号+6位年月+4位随机数字！'
    },
    jqbh:{
    	validator:function(value,param){
    		if(Trim(value,'be')==''){
    	...
    	message:'为J开头+12位部门编号+6位年月+4位随机数字！'
    },
    /**
     *@ 英文格式校验
     *@ value 验证文本
